...
from model_hgnn import embed_net
from utils import *
from loss import OriTripletLoss
from loss import QuarCenterTripletLoss as QuadCenterTripletLoss


# from loss import MRICs
...
parser.add_argument('--optim', default='SGD_1', type=str, help='SGD,ADM')
parser.add_argument('--model_path', default='result/WCHG-Net-2/save_model/', type=str, help='model save path')
parser.add_argument('--log_path', default='result/WCHG-Net-2/log/', type=str, help='log save path')
parser.add_argument('--vis_log_path', default='result/WCHG-Net-2/log/vis_log/', type=str, help='log save path')
...
parser.add_argument('--module', default='sysu',help='dataset name: regdb or sysu or llcm]')
parser.add_argument('--wave_bands', default=3, type=int, help='CMGR Chebyshev high-frequency band count')
...
DATA_ROOT = os.environ.get('WCHG_DATA_ROOT', './dataset')


def dataset_path(name):
    return os.path.join(DATA_ROOT, name) + os.sep


if dataset == 'sysu':

    data_path = dataset_path('SYSU-MM01')
    log_path = args.log_path + 'sysu_log/'
    test_mode = [1, 2]  # thermal to visible
elif dataset == 'regdb':
    data_path = dataset_path('RegDB')
    log_path = args.log_path + 'regdb_log/'
    test_mode = [2, 1]  # visible to thermal
elif dataset == 'llcm':
    data_path = dataset_path('LLCM')
...
suffix = dataset
suffix = suffix + '_module_{}_wchgnet_p{}_n{}_lr_{}_seed_{}'.format(
    args.module,args.num_pos, args.batch_size, args.lr, args.seed)
...
if args.backbone == 'AGW':
    embeding_dim = 2048
    net = embed_net(n_class, no_local='on', gm_pool='on', arch=args.arch, wave_bands=args.wave_bands)
else:
    embeding_dim = 2048
    net = embed_net(n_class, no_local='on', gm_pool='on', arch=args.arch, wave_bands=args.wave_bands)
...
if args.loss_tri == 'QCT':
    ...
    criterion_qct = QuadCenterTripletLoss(k_size=4, margin=0.7)

else:
    loader_batch = args.batch_size * args.num_pos
    criterion_qct = OriTripletLoss(batch_size=loader_batch, margin=args.margin)


criterion_id.to(device)
criterion_qct.to(device)
...
def train(epoch):
    ...
    lqct_loss = AverageMeter()
    lher_loss = AverageMeter()
    lhor_loss = AverageMeter()
    ...
    for batch_idx, (input10, input11, input20, input21, label1, label2) in enumerate(trainloader):
        ...
        with amp.autocast(enabled=True):
            ...
            feat, out0, loss_dict = net(input10, input11, input20, input21)

            loss_lid = criterion_id(out0, labels)
            loss_lqct = criterion_qct(feat, labels)
            zero = torch.tensor(0.0, device=labels.device)
            loss_lher = loss_dict.get('Lher', zero)
            loss_lhor = loss_dict.get('Lhor', zero)

            _, predicted = out0.max(1)
            correct_id += (predicted.eq(labels).sum().item())

            loss = loss_lid + loss_lqct + loss_lher + loss_lhor
        ...
        lid_loss.update(loss_lid.item(), 2 * input10.size(0))
        lqct_loss.update(loss_lqct.item(), 2 * input10.size(0))
        lher_loss.update(loss_lher.item(), 2 * input10.size(0))
        lhor_loss.update(loss_lhor.item(), 2 * input10.size(0))
        ...
        if batch_idx % 50 == 0:
            print('Epoch: [{}][{}/{}] '
                  ...
                  'Lid: {lid.val:.4f} ({lid.avg:.4f}) '
                  'Lqct: {lqct.val:.4f} ({lqct.avg:.4f}) '
                  'Lher: {lher.val:.4f} ({lher.avg:.4f}) '
                  'Lhor: {lhor.val:.4f} ({lhor.avg:.4f}) '
                  'Accu: {:.2f} '.format(
                      epoch, batch_idx, len(trainloader), current_lr,
                      100. * correct_id / total, batch_time=batch_time,
                      train_loss=train_loss, lid=lid_loss, lqct=lqct_loss,
                      lher=lher_loss, lhor=lhor_loss))
    scdule.step()
    scdule_shallow.step()

    writer.add_scalar('total_loss', train_loss.avg, epoch)
    writer.add_scalar('Lid', lid_loss.avg, epoch)
    writer.add_scalar('Lqct', lqct_loss.avg, epoch)
    writer.add_scalar('Lher', lher_loss.avg, epoch)
    writer.add_scalar('Lhor', lhor_loss.avg, epoch)
...
for epoch in range(start_epoch, 150 - start_epoch):
    ...
    if epoch > 0 and epoch % 1 == 0:
        ...
        # save model (best by FC mAP)

        if mAP_att > best_map_fc:
            best_map_fc = mAP_att
            best_epoch = epoch
            state = {
                'net': net.state_dict(),
            ...
            torch.save(state, checkpoint_path + suffix + '_best_fc_mAP.t')
