...
# from model_agp_spatial import embed_net
# from model_agpw_copy import embed_net
from model_hgnn import embed_net


# from model import embed_net

# from model_anm import embed_net
...
def _remap_legacy_wchgnet_keys(state_dict):
    """Map pre-CMGR checkpoint keys to the current CMGR module names."""
    remapped = {}
    for key, value in state_dict.items():
        if key.startswith('hg3.'):
            key = 'cmgr.hypergraph.' + key[len('hg3.'):]
        elif key.startswith('hg3_wave.'):
            key = 'cmgr.spectral_readout.' + key[len('hg3_wave.'):]
        elif key.startswith('gmg.gnn.'):
            key = key.replace('gmg.gnn.', 'cmgr.graph_readout.gcn.', 1)
            key = key.replace('.lin.', '.proj.')
            key = key.replace('.ln.', '.norm.')
        elif key.startswith('gmg.proj.'):
            key = 'cmgr.graph_readout.proj.' + key[len('gmg.proj.'):]
        elif key.startswith('hg3_diff.'):
            continue
        remapped[key] = value
    return remapped


def load_checkpoint_weights(net, checkpoint):
    state_dict = checkpoint['net'] if isinstance(checkpoint, dict) and 'net' in checkpoint else checkpoint
    state_dict = _remap_legacy_wchgnet_keys(state_dict)
    msg = net.load_state_dict(state_dict, strict=False)
    if msg.missing_keys:
        print('Missing keys after compatible load:', msg.missing_keys)
    if msg.unexpected_keys:
        print('Unexpected keys after compatible load:', msg.unexpected_keys)
...
parser.add_argument('--resume', '-r', default='sysu_wchgnet_p4_n6_lr_0.1_seed_0_best.t', type=str,
                    help='resume from checkpoint')
parser.add_argument('--model_path', default='result/WCHG-Net-2/save_model/',
                    type=str, help='model save path')
parser.add_argument('--log_path', default='result/WCHG-Net-2/log/',
                    type=str, help='log save path')
parser.add_argument(
    '--vis_log_path', default='result/WCHG-Net-2/log/vis_log/', type=str, help='log save path')
...
def extract_gall_feat(gall_loader):
    ...
    with torch.no_grad():
        for batch_idx, (input, label) in enumerate(gall_loader):
            ...
            input1 = Variable(input.to(device))  # 修改为 to(device)
            input2 = Variable(fliplr(input).to(device))  # 修改为 to(device)
...
if dataset == 'sysu':
    ...
    if len(args.resume) > 0:
        ...
        if os.path.isfile(model_path):
            ...
            load_checkpoint_weights(net, checkpoint)
            net.to(device)
    ...
    for trial in range(10):
        ...
        resume_name = args.resume.format(test_trial) if '{}' in args.resume else args.resume
        model_path = checkpoint_path + resume_name
