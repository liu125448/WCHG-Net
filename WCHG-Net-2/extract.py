...
#模型名
parser.add_argument('--resume', '-r', default='your_checkpoint.t', type=str, help='resume from checkpoint')
...
net = embed_net(n_class, no_local='on', gm_pool='on', arch=args.arch)#基线用off，模型用on
...
def extract_gall_feat(gall_loader):
    ...
    with torch.no_grad():
        for batch_idx, (input, label ) in enumerate(gall_loader):
            ...
            feat_pool, feat_fc = net(input, input, input, input, test_mode[0])
...
def extract_query_feat(query_loader):
    ...
    with torch.no_grad():
        for batch_idx, (input, label ) in enumerate(query_loader):
            batch_num = input.size(0)
            input = Variable(input.cuda())
            feat_pool, feat_fc = net(input, input, input, input, test_mode[1])
...
if dataset == 'llcm':
    ...
    if len(args.resume) > 0:
        ...
        if os.path.isfile(model_path):
            print('==> loading checkpoint {}'.format(args.resume))
            checkpoint = torch.load(model_path, map_location={"cuda:4": "cuda:0"})
            net.load_state_dict(checkpoint['net'])
            print('==> loaded checkpoint {} (epoch {})'
                  .format(args.resume, checkpoint['epoch']))
        else:
    ...
    result = {'gallery_f':gall_feat_fc,'gallery_label':gall_label,'gallery_cam':gall_cam,'query_f':query_feat_fc,'query_label':query_label,'query_cam':query_cam}
    scipy.io.savemat('tsne_ours.mat',result) #保存在代码主目录下，跟model.py同级
